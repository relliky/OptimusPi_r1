...
void I2CTestClass::runBasicReadTest(void)
{
	...
	I2C_UUT.readRegisters8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_WHO_AM_I, &TP_Rreg8_signle, 1);
	while(TP_Rreg8_signle != 0x68){}
	//pass

	//1.1.2. test readReg8 multiple byte functionality
	//test the second reg
	I2C_UUT.readRegisters8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_WHO_AM_I - 1, TP_Rreg8_multiple_arr, 2);
	while(TP_Rreg8_multiple_arr[1] != 0x68){}
	//pass

	//1.2.1 test readReg16 single word functionality
	I2C_UUT.readRegisters16(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_WHO_AM_I - 1, &TP_Rreg16_single, 1);
	...
	I2C_UUT.readRegisters16(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_WHO_AM_I - 3, TP_Rreg16_multiple_arr, 2);
	...
	I2C_UUT.readBit8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_WHO_AM_I, 3, &TP_readBit8);
	...
	I2C_UUT.readbits8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_WHO_AM_I, 5, 3, &TP_readbits8);
...
void I2CTestClass::runBasicWriteTest(void)
{
	...
	uint8_t TP_test_reg_reset 	= 0		,
			...
			TP_Wbit_data_read	= 0		,
			TP_WBits_data		= 0x3	,
			TP_WBits_data_read  = 0		;


	//2.0 check the reset value of the test reg for following testing.
	I2C_UUT.writeRegisters8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_INT_PIN_CFG, &TP_test_reg_reset, 1);
	I2C_UUT.readRegisters8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_INT_PIN_CFG, &TP_test_reg_reset, 1);
	...
	I2C_UUT.writeRegisters8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_INT_PIN_CFG, &TP_test_reg_set, 1);
	//and read back
	I2C_UUT.readRegisters8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_INT_PIN_CFG, &TP_Wreg8_single, 1);
	...
	I2C_UUT.writeRegisters8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_INT_PIN_CFG - 1, TP_Wreg8_mul_arr, 2);
	//and read back the second byte written
	I2C_UUT.readRegisters8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_INT_PIN_CFG, &TP_Wreg8_mut_read, 1);
	...
	I2C_UUT.readRegisters8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_INT_PIN_CFG, &TP_test_reg_set, 1);
	while(TP_test_reg_reset != 0x00){}
	//write a 0 to bit 5
	I2C_UUT.writeBit8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_INT_PIN_CFG, 5, TP_Wbit_data);
	//read back and check if bit 5 is written as 0
	I2C_UUT.readRegisters8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_INT_PIN_CFG, &TP_Wbit_data_read, 1);
	...
	I2C_UUT.writeRegisters8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_INT_PIN_CFG, &TP_test_reg_reset, 1);
	while(TP_test_reg_reset != 0x00){}
	//write a 0b11 into bit 5 and bit 6
	I2C_UUT.writeBits8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_INT_PIN_CFG, 6, 2, TP_WBits_data);
	//read back and check if bit 5 and bit 6 are written as 0b11
	I2C_UUT.readRegisters8(MPU9150_ADDRESS_AD0_LOW, MPU9150_RA_INT_PIN_CFG, &TP_WBits_data_read, 1);
	while(TP_WBits_data_read != 0x60){}
	//pass
