 ...
 *      Author: Tai
 *
 *  Comments: The file itself cannot be called Debug.cpp, otherwise the compiler would not find the symbol for any method within this class. It might be a linker issue and the reason could be there is another file called Debug.cpp.
 */
...
void DebugFunctionClass::RunTests(void)
{
	printf("\n\n");
			...
			for(int i=0; i<MPUTestLoopingTimes; i++)
			{
				printf("     This is the %dth time taking the data out of %d times\n", i, MPUTestLoopingTimes);
